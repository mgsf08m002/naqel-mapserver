(function () {
    ...
    function requesterLabel(item) {
        ...
        return escapeHtml(item.requester.name || item.requester.username);
    }
    ...
    function setActiveScope(btn) {
        if (!scopeRoot) return;
        scopeRoot.querySelectorAll('.review-history-scope-btn').forEach(function (b) {
