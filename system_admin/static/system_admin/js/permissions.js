    ...
    function renderClassicView() {
        ...
        container.innerHTML = state.filteredUsers.map(user => {
            ...
            const actionButtonClass = user.has_permissions 
                ...
                : 'rounded-full border-2 border-geotrak bg-geotrak px-4 py-2 text-xs font-semibold text-white hover:bg-geotrak-hover focus:outline-none focus:ring-2 focus:ring-gray-200 transition-all duration-200';

            return `
                <div class="bg-white rounded-lg border-2 border-gray-200 p-4 sm:p-5 shadow-sm hover:shadow-md transition-shadow">
                    <div class="space-y-3">
                        <div>
                            <h3 class="text-sm font-semibold text-geotrak">${escapeHtml(user.full_name || user.email)}</h3>
        ...
        tbody.innerHTML = state.filteredUsers.map(user => {
            ...
            return `
                <tr class="border-b border-gray-100 hover:bg-gray-50 transition-colors duration-150">
                    <td class="py-3 px-3 sm:px-4 text-xs sm:text-sm font-semibold text-geotrak whitespace-nowrap">
                        ${escapeHtml(user.full_name || user.email)}
                    </td>
                    <td class="py-3 px-3 sm:px-4 text-xs sm:text-sm text-gray-700 whitespace-nowrap">
                        ${escapeHtml(user.email)}
                    </td>
                    <td class="py-3 px-3 sm:px-4 text-xs sm:text-sm font-medium text-geotrak whitespace-nowrap">
